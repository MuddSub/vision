...
def filter(image,blkSize=10*10,patchSize=8,lamb=10,gamma=1.7,r=10,eps=1e-6,level=5):
	...
	fTrans = 0.6
	trans = TEtransEstimate(AL,patchSize,airlight,lamb,fTrans,r,eps,gamma)
	show2(trans*255,"final transmission map",False) # work!
	
	AL = dehazeProcess(AL,trans,airlight) #correct, np array adding matters!
...
def dehazeProcess(img, trans, airlight):#different,maybe give the other alg-->simplestColor another try
	global lf
	lf=[]
...
def IDbuildGaussianPyramid(img,level):
	...
	for i in range(1,level):
		tmpImg = cv2.resize(src=tmpImg,dsize = None,fx= 0.5,fy=0.5,interpolation =cv2.INTER_LINEAR)
...
def IDbuildLaplacianPyramid(img,level):
	...
	for i in range(0,level-1):
		...
		tmpPyr = cv2.resize(src=lapPyr[i+1],dsize =(y,x),fx= 0,fy=0,interpolation =cv2.INTER_LINEAR)
		#lb.append(lapPyr[i])	
		#lg.append(tmpPyr)
...
def TEcomputeTrans(img,patchSz,rows,cols,type,airlight,lamb,fTrans):
	...
	for i in range(0,rows,patchSz):
		for j in range(0,cols,patchSz):
			...
			Trans = BTEblkEstimate(blkIm,airlight,lamb,fTrans)# done, checked. 
			la.append(Trans)
...
def BTEblkEstimate(blkIm,airlight,lamb,fTrans):
	...
	while(nCounter<(1-fTrans)*10):
		...
		nSumOfSquareOuts = np.sum(np.multiply(bChannel,bChannel))+np.sum(np.multiply(gChannel,gChannel))+np.sum(np.multiply(rChannel,rChannel))
		nSumOfOuts = np.sum(bChannel)+np.sum(gChannel)+np.sum(rChannel)
		fMean = nSumOfOuts/numberOfPixels
		fCost = lamb * nSumOfLoss / numberOfPixels - (nSumOfSquareOuts/numberOfPixels - fMean**2)
...
def FguidedImageFilter(I,p,r,eps):
	...
	N = cv2.boxFilter( np.ones((I.shape[0],I.shape[1]),dtype=I[0].dtype), -1, (r,r))
...
def FsimpleColorBalance(img,percent):
	...
	for i in range(chnls):
		...
		channels[i]= cv2.normalize(channels[i],channels[i],0.0,255.0/2,cv2.NORM_MINMAX)
...
def FWsaliency(img):
	...
	l,a,b = np.array(l,dtype=np.float32),np.array(a,np.float32),np.array(b,np.float32)
