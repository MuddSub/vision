...
def mainImg(img):
	...
	lineLocs, certainty = convolute(newImg1,True)
...
def convolute(img,graph):
	height = 20
	print(height)
	kernel_0 = np.array([ copy.deepcopy([-1,1]) for i in range(height) ])
	kernel_1 =np.array([ copy.deepcopy([-1,1]) for i in range(height) ])
	
	newImg = copy.deepcopy(img)

	img_0 = cv2.filter2D(img, -1, kernel_0)
	show(img_0,"img 0",False)	
	img_1 = cv2.filter2D(img, -1, kernel_1)

	lineLocs = []
	csums_0 = np.sum(img_0,axis=0)
	csums_1 = np.sum(img_1,axis=0)

	lineLocs.append( [ np.argmin( csums_0 ), csums_0[ np.argmin( csums_0 ) ]]) 
	
	lineLocs.append( [ np.argmin( csums_1 ), csums_1[ np.argmin( csums_1 ) ]]  )
	print(lineLocs)
