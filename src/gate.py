...
class Gate:
    ...
    def rotateGetLines(self,image,graph):
        ...
        image = self.rotateToHorizontal(image,lb,ub,delta)
        k = self.getLines(image)
        if k != -1:
            lineLocs.append(k)
        leeway = 40
        if len(lineLocs)==1:
            ...
            numDetected = 0
            return lineLocs,0
        image = self.rotateToHorizontal(o,lb,ub,delta)
        k = self.getLines(image)
        if k!= -1:
            lineLocs.append(k)
        if len(lineLocs) == 2:
    ...
    def rotateToHorizontal(self,img, lb=-20, ub=20, incr=2, topN=1):
        bestscore = -np.inf
        bestTheta = 0
        img = 255 - img
    ...
    def findBars(self,img):
        ...
        segmented = self.adjust(segmented)
